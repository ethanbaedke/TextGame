...
case "$2" in
        "CR_STR")
            value=$(dd if=data/characters/$1-data.bin bs=1 skip=0 count=1 status=none)
            exit "$value"
            ;;
        "CR_DEX")
            value=$(dd if=data/characters/$1-data.bin bs=1 skip=1 count=1 status=none)
            exit "$value"
            ;;
        "CR_INT")
            value=$(dd if=data/characters/$1-data.bin bs=1 skip=2 count=1 status=none)
            exit "$value"
            ;;
esac